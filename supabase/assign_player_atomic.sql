-- =============================================================================
-- Migration: assign_player_atomic RPC
-- Run in Supabase Dashboard → SQL Editor → New query
-- Fixes the concurrent sign-in race conditions in autoAssignPlayer.
--
-- What this solves:
--   • Two players arriving simultaneously both tried to claim the same role
--     (TOCTOU: read open slots → pick role → write — not atomic).
--   • joinGame used read-modify-write which silently dropped the first writer's
--     player when two concurrent writes landed on the same game row.
--
-- How it works:
--   • Locks session_settings FOR UPDATE — serializes ALL concurrent logins.
--   • Checks existing assignment by email across all active games (FOR UPDATE).
--   • Finds the first lobby game with a free human slot (FOR UPDATE on game row).
--   • Atomically writes the new player via JSONB || operator (single UPDATE).
--   • If no open lobby game, returns needsCreate=true so the client creates
--     a fresh team (client-side; acceptable minor race for first-player-in-team).
-- =============================================================================
...
BEGIN
  ...
  -- ── 4b. No open game — tell client to create a new team ───────────────────
  -- Client creates the game row (needs TypeScript for createInitialGameState).
  -- Minor race: two clients getting needsCreate=true in rapid succession each
  -- create their own 1-person team. For workshops this window is < 10 ms and
  -- the retry in autoAssignPlayer catches most cases.
  SELECT COUNT(*) + 1 INTO v_team_num FROM games;
